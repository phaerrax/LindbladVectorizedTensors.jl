...
function premultiply(mat, ::SiteType"vFermion")
    return _hilbertschmidt_vec(x -> mat * x, gellmannbasis(2))
end
function postmultiply(mat, ::SiteType"vFermion")
    return _hilbertschmidt_vec(x -> x * mat, gellmannbasis(2))
end
...
function ITensors.op(on::OpName, st::SiteType"vFermion"; kwargs...)
    ...
    if !isnothing(dotloc)
        ...
        if on1 == ""
            mat = matrix(op(on2, siteind("Fermion"); kwargs...))
            return postmultiply(mat, st)
        elseif on2 == ""
            mat = matrix(op(on1, siteind("Fermion"); kwargs...))
